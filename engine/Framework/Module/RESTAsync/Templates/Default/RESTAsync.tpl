...
{foreach from=$links item=object}
	{if $object.object_type == 'Item'}
		<item id="{$object.item_id}" locationID="{$object.location_id}" name="{$object.name}" type="{$object.type}" description="{$object.description}" icon="{$object.icon}" media="{$object.mediaURL}" />
	{else}
